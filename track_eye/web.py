...
async function poll(){try{const r=await fetch('/status.json',{cache:'no-store'});const s=await r.json();document.getElementById('face').textContent=s.face_detected?'OK':'NO FACE';document.getElementById('left').textContent=s.left?s.left.x.toFixed(3)+', '+s.left.y.toFixed(3):'--';document.getElementById('right').textContent=s.right?s.right.x.toFixed(3)+', '+s.right.y.toFixed(3):'--';statusEl.textContent=JSON.stringify(s,null,2)}catch(e){statusEl.textContent='status unavailable'}}
